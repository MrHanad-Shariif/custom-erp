          ...
          <div class="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
            ...
            <div>
              ...
              <form @submit.prevent="handleSubmit">
                <div class="space-y-5">
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Organization Name *</label>
                    <input v-model="organizationName" type="text" placeholder="Your company name"
                      class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Organization Code *</label>
                    <input v-model="organizationCode" type="text" placeholder="e.g. ACME"
                      class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                  </div>
          ...
          <div class="flex items-center justify-center z-1">
            ...
            <div class="flex flex-col items-center max-w-xs">
              <p class="text-center text-gray-400 dark:text-white/60">
                Halabuur ERP is Custom erp designed to make you work easy
              </p>
            </div>
          </div>
        </div>
...
const googleSignInUrl = computed(() => {
  const redirectUri = typeof window !== 'undefined' ? `${window.location.origin}/signin/callback` : `${import.meta.env.VITE_APP_ORIGIN || 'http://localhost:5173'}/signin/callback`
  return `${apiBase}/api/auth/google?redirect_uri=${encodeURIComponent(redirectUri)}`
})
...
const handleSubmit = async () => {
  ...
  if (!organizationName.value?.trim() || !organizationCode.value?.trim() || !fullName || !email.value || !password.value) {
    errorMessage.value = 'Organization name, code, name, email and password are required'
    return
  }
  loading.value = true
  try {
    ...
    const r = await fetch(base + '/api/auth/register', {
      ...
      body: JSON.stringify({
        ...
        organization_code: organizationCode.value.trim().toUpperCase(),
        full_name: fullName,
    ...
    const d = res.data as { user: { id: string; full_name: string; email: string }; organization: { id: string; name: string; code: string }; access_token: string; refresh_token: string }
    setSession(d.access_token, d.refresh_token, d.user as any, d.organization as any, [])
